...
void OpenManipulatorTeleop::setGoal(char ch)
{
  ...
  else if (ch == '1')
  {
    ...
    joint_name.push_back("joint4"); joint_angle.push_back(0.350);
    setJointSpacePath(joint_name, joint_angle, path_time);
  }
}
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "open_manipulator_auto_position");
  ros::NodeHandle nh;
  OpenManipulatorTeleop openManipulatorTeleop;

  // Define the rate at which to loop
  ros::Rate loop_rate(0.5); // 0.5 Hz, or once every 2 seconds, to account for the 4-second delay within each cycle

  while (ros::ok())
  {
    // Go to home position
    openManipulatorTeleop.setGoal('2');
    ros::Duration(4).sleep(); // Wait for 4 seconds at home position

    // Make sure callbacks are processed
    ros::spinOnce();

    // Go to initial position
    openManipulatorTeleop.setGoal('1');
    ros::Duration(4).sleep(); // Wait for 4 seconds at initial position

    // Make sure callbacks are processed
    ros::spinOnce();

    // Control the loop rate (optional, depending on your exact requirements)
    loop_rate.sleep();
  }
